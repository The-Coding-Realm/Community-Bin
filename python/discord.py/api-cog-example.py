...
class MyApiCog(commands.Cog):
    ...
    def __init__(self, bot):
        ...
        self._available_animals = (
            "dog", "cat", "panda", "red_panda", "fox", "birb", "bird", "koala", "whale", "racoon", "kangaroo")
    ...
    async def _canvas_to_file(self, *, endpoint, member: discord.Member,
                              img_type="png"):  # converts canvas endpoint to discord.File
        async with self.session.get(
                self.base_url + f"canvas/{endpoint}?avatar={member.avatar_url_as(format='png', size=1024)}") as f:
    ...
    async def _birb_fact(self, ctx):
        _animal_fact = await self._get_animal_fact("bird")  # bird for facts and birb for img
